...
public class PlayerController : MonoBehaviour {
	...
	public string horiz;
	public string vert;
	public string jumpBut, jumpBut2, jumpBut3;
	public string attackBut;
	public string throwBut;
...
//				Debug.Log (Input.GetAxis (vert));
				if ((Input.GetAxis (vert) < -0.85 || Input.GetKeyDown (jumpBut2) || Input.GetKeyDown (jumpBut3)) && jumpCount < 2 && Time.time > nextJump) {
			...
			else if (Input.GetKeyDown (jumpBut) && jumpCount < 2 && Time.time > nextJump) {
				nextJump = Time.time + jumpCoolDown;
				jumpSound.Play ();
				rb.AddForce (vertJump);
				jumpCount++;
...
//				rb.AddForce (expAngle * 300, ForceMode2D.Impulse);
	...
	private void OnTriggerStay2D(Collider2D other) {
		if (other.gameObject.tag == "ItemSpawn") {
			if (curWeapon == null && Input.GetKeyUp (throwBut) && other.GetComponent<ItemSpawnController>().curItemExists()) {
				pickedUpItem = true;
